...
from app.models.voice.features import load_audio, extract_mfcc, compute_zero_crossing_rate, compute_rms

def main():
    parser = argparse.ArgumentParser(description="Train voice emotion model")
    parser.add_argument("--data-dir", default="data/raw/voice", help="Directory containing emotion subfolders")
    parser.add_argument("--out-model", default="models/voice_emotion.pkl", help="Output model path")
    parser.add_argument("--limit-per-class", type=int, default=0, help="Limit samples per class for faster training")
