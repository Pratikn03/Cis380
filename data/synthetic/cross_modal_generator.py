                ...
                ModalitySignal(
                    ...
                    features={
                        "request_type": random.choice(
                            ["wire_transfer", "password_reset", "account_update"]
                        ),
                        "request_amount": (
                            random.uniform(1000, 50000) if random.random() > 0.3 else None
                        ),
...
class CrossModalDataGenerator:
    ...
    def export_to_json(self, samples: List[CrossModalSample], output_path: Path) -> None:
        ...
        for sample in samples:
            ...
            sample_dict["signals"] = [
                asdict(s) if hasattr(s, "__dataclass_fields__") else s for s in sample.signals
            ]
    ...
    def export_to_pytorch_format(
        self, samples: List[CrossModalSample], output_dir: Path
    ) -> Dict[str, Any]:
        ...
        modality_dims = {"fraud": 10, "cyber": 10, "behavior": 10, "vision": 10, "voice": 10}
...
def main():
    ...
    train_samples = generator.generate_dataset(n_samples=1000, anomaly_ratio=0.3)
    ...
    val_samples = generator.generate_dataset(n_samples=200, anomaly_ratio=0.3)
    ...
    # Generate test set
    test_samples = generator.generate_dataset(n_samples=200, anomaly_ratio=0.3)
