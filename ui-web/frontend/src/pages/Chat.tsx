...
const convertBlobToWavFile = async (
  blob: Blob,
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        convertBlobToWavFile(rawBlob)
          .then((file) => {
            setSelectedFile(file);
            appendMessage({
              role: "assistant",
              content: "🎤 Audio recorded! Click 'Run Analysis' to analyze emotion.",
            });
          })
          .catch(() => {
            const fallbackFile = new File([rawBlob], "recording.webm", {
              type: rawBlob.type || "audio/webm",
            });
            setSelectedFile(fallbackFile);
            appendMessage({
              role: "assistant",
              content: "⚠️ Audio recorded but could not convert to WAV. Try uploading a WAV file.",
            });
          });
      };
