...
class NoiseGenerator:
    ...
    def add_noise_at_snr(self, signal: Any, noise: Any, snr_db: float) -> Any:
    ...
    def predict(self, audio: Any, noise_level: float = 0.0) -> Dict[str, Any]:
        ...
        if np is not None:
            ...
            predicted = (
                ...
                else random.choice(self.EMOTIONS)
            )
...
class VoiceRobustnessTester:
    ...
    def __init__(self, model: Any = None, sample_rate: int = 16000):
    ...
    def run_test(
        ...
        if audio_samples is None:
            ...
            if np is not None:
                audio_samples = [
                    np.random.randn(int(duration * self.sample_rate)) * 0.5
                    for _ in range(n_samples)
                ]
            else:
                audio_samples = [
                    [random.gauss(0, 0.5) for _ in range(int(duration * self.sample_rate))]
                    for _ in range(n_samples)
                ]
        ...
        for noise_type in noise_types:
            for snr_db in snr_levels:
                result = self._test_condition(audio_samples, noise_type, snr_db)
    ...
    def _test_condition(
        self, samples: List[Any], noise_type: str, snr_db: float
    ) -> RobustnessResult:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "snr_range": [
                    min(r.snr_db for r in self._results),
        ...
        for noise_type in set(r.noise_type for r in self._results):
            ...
            report["by_noise_type"][noise_type] = {
                ...
                "results": [
                    r.to_dict() for r in sorted(type_results, key=lambda r: r.snr_db, reverse=True)
                ],
            }
...
class TestVoiceRobustnessTester:
    ...
    def test_accuracy_degrades_with_noise(self):
        ...
        tester = VoiceRobustnessTester()
        results = tester.run_test(n_samples=50, noise_types=["white"], snr_levels=[30, 10, 0, -5])
...
if __name__ == "__main__":
    ...
    results = tester.run_test(
        n_samples=50, noise_types=["white", "pink", "babble"], snr_levels=[30, 20, 10, 5, 0]
    )
    ...
    for r in results:
        print(
            f"{r.noise_type:8} @ {r.snr_db:3}dB: "
