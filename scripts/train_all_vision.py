...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Train all vision models (brand + deepfake + real/fake + video)."
    )

    parser.add_argument(
        "--skip-full", action="store_true", help="Skip deepfake + real/fake image training."
    )
    parser.add_argument(
        "--skip-brand", action="store_true", help="Skip brand/logo detector training (YOLO)."
    )
    parser.add_argument(
        "--skip-temporal", action="store_true", help="Skip video temporal model training."
    )

    # Forwarded to scripts/train_all_vision_full.py when enabled.
    parser.add_argument(
        "--device", default="auto", help="auto|cpu|cuda|mps (forwarded to full training)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="DataLoader workers (forwarded to full training).",
    )
    parser.add_argument(
        "--val-split", type=float, default=0.15, help="Val split for real/fake (forwarded)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Output dir for vision weights.",
    )
    parser.add_argument(
        "--celeb-root", default=str(REPO_ROOT / "data" / "Celeb_V2"), help="Celeb_V2 dataset root."
    )
    parser.add_argument(
        "--raw-vision-root",
