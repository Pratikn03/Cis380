...
def extract_features_simple(audio_path, max_len=128):
    ...
    try:
        ...
        spectral_bandwidth = np.sqrt(
            np.sum(((freqs - spectral_centroid) ** 2) * fft_norm) / (np.sum(fft_norm) + 1e-8)
        )
...
def extract_features_librosa(audio_path, max_len=128):
    ...
    try:
        ...
        extra = np.concatenate(
            [mfcc_mean, mfcc_std, chroma_mean, [spec_cent, spec_bw, spec_rolloff, zcr, rms]]
        )
...
print("✅ Saved: models/voice_emotion_nn.pt")

# Also train sklearn MLP for backward compatibility
print("\n📦 Training sklearn MLP backup model...")
mlp = MLPClassifier(
    hidden_layer_sizes=(256, 128, 64), max_iter=300, early_stopping=True, random_state=42
)
...
joblib.dump(
    {"model": mlp, "scaler": scaler, "label_encoder": le}, PROJECT_ROOT / "models/voice_emotion.pkl"
)
print("✅ Saved: models/voice_emotion.pkl")
