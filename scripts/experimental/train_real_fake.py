...
def train_real_fake_classifier(max_images_per_class=40000, epochs=3, batch_size=32):
    ...
    train_data, val_data = train_test_split(
        all_images, test_size=0.2, random_state=42, stratify=[x[1] for x in all_images]
    )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if batch_idx % 200 == 0:
                ...
                print(
                    f"   Epoch {epoch+1}/{epochs} | Batch {batch_idx}/{len(train_loader)} | Acc: {acc:.1f}%"
                )
    ...
    print("   Model saved: models/vision/real_fake_classifier.pt")

    # Save class names
    classes_file = PROJECT_ROOT / "models/vision/real_fake_classes.txt"
    with open(classes_file, "w") as f:
        f.write("real\nfake\n")
    print("   Classes saved: models/vision/real_fake_classes.txt")
...
if __name__ == "__main__":
    ...
    train_real_fake_classifier(
        max_images_per_class=args.max_images, epochs=args.epochs, batch_size=args.batch_size
    )
