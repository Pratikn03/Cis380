...
def _readiness_status(checks: list[Check], *, fail_on: tuple[str, ...] = ("missing",)) -> str:
    statuses = {check.status for check in checks}
    if any(status in fail_on for status in statuses):
        return BLOCKED
    if "warn" in statuses:
        return DEGRADED
    return READY
...
def main() -> int:
    ...
    core_payload = {
        ...
        "readiness_status": _readiness_status(core_checks, fail_on=("missing",)),
        "blocking": _readiness_status(core_checks, fail_on=("missing",)) == BLOCKED,
    ...
    extended_payload = {
        ...
        "readiness_status": _readiness_status(extended_checks, fail_on=("missing",)),
        "blocking": _readiness_status(extended_checks, fail_on=("missing",)) == BLOCKED,
    ...
    core_payload["status_vocabulary"] = {
        "canonical": ["ready", "degraded", "blocked", "advisory"],
        "legacy": ["green", "red", "warn"],
    }
    extended_payload["status_vocabulary"] = core_payload["status_vocabulary"]

    _write(REPORTS / "training_preflight_core.json", core_payload)
    _write(REPORTS / "training_preflight_extended.json", extended_payload)
    print(
        json.dumps(
            {"core": core_payload["readiness_status"], "extended": extended_payload["readiness_status"]},
            indent=2,
        )
    )
    return 0
