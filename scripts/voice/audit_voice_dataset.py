...
def audit_dataset(root: Path, emotions: list[str], speaker_pattern: re.Pattern[str]) -> dict[str, Any]:
    summary: dict[str, Any] = {
        "root": str(root),
        "voice_source": "processed_balanced" if "processed" in str(root) and "voice_balanced" in str(root) else "raw",
        "classes": {},
        "total_files": 0,
        "total_speakers": 0,
        "speaker_counts": {},
...
def main() -> None:
    ...
    canonical_root = args.data_root
    balanced_root = Path("data/processed/voice_balanced")
    if args.data_root == Path("data/raw/voice") and balanced_root.exists():
        canonical_root = balanced_root
    report = audit_dataset(canonical_root, emotions, pattern)
    report["source_root_raw"] = str(Path("data/raw/voice"))
    report["source_root_balanced"] = str(balanced_root)
    report["voice_source_resolution"] = "processed_balanced" if canonical_root == balanced_root else "raw"
    report["readiness_status"] = "ready" if report["total_files"] > 0 else "blocked"
