...
def train_fraud_model():
    ...
    clf = Pipeline([
        ...
        ('clf', StackingClassifier(
            estimators=[
                ('rf', RandomForestClassifier(n_estimators=300, max_depth=25, random_state=42, n_jobs=-1, class_weight='balanced')),
                ('gb', GradientBoostingClassifier(n_estimators=200, max_depth=10, learning_rate=0.1, random_state=42)),
                ('hgb', HistGradientBoostingClassifier(max_iter=300, max_depth=12, learning_rate=0.1, random_state=42)),
            ],
    ...
    clf = Pipeline([
        ...
        ('clf', StackingClassifier(
            estimators=[
                ...
                ('gb', GradientBoostingClassifier(n_estimators=200, max_depth=12, learning_rate=0.1, random_state=42)),
                ('hgb', HistGradientBoostingClassifier(max_iter=300, max_depth=15, learning_rate=0.1, random_state=42)),
            ],
...
def train_voice_emotion_model():
    ...
    clf = Pipeline([
        ...
        ('clf', StackingClassifier(
            estimators=[
                ('rf', RandomForestClassifier(n_estimators=400, max_depth=35, random_state=42, n_jobs=-1)),
                ('gb', GradientBoostingClassifier(n_estimators=250, max_depth=12, learning_rate=0.1, random_state=42)),
...
def train_text_classifier():
    ...
    texts = [
        # Positive
        "I love this product, it's amazing!", "Great quality and fast shipping",
        "Excellent service, highly recommend", "Best purchase I ever made",
        "Wonderful experience, will buy again", "Perfect, exactly what I needed",
        "Outstanding quality, exceeded expectations", "Fantastic product, love it",
        "Amazing customer service", "Highly satisfied with my purchase",
        "This is incredible, so happy", "Superb quality, worth every penny",
        "Brilliant product, works perfectly", "Absolutely love this, thank you",
        "Best decision ever, so glad I bought this", "Phenomenal results, amazing",
        # Negative
        "Terrible product, complete waste of money", "Worst purchase ever, avoid",
        "Very disappointed with the quality", "Product broke after one day",
        "Horrible customer service", "Would not recommend to anyone",
        "Complete garbage, don't buy", "Extremely poor quality, returning it",
        "Awful experience, never again", "Waste of time and money",
        "Defective product, very frustrated", "Terrible, worst I've ever seen",
        "Broken on arrival, unacceptable", "Disappointing quality, not as described",
        "Regret buying this, total scam", "Horrible, want my money back",
        # Neutral
        "The product is okay, nothing special", "Average quality, fair price",
        "It works as expected, nothing more", "Decent product for the price",
        "Standard quality, meets expectations", "Acceptable, does the job",
        "Mediocre quality, could be better", "Fair product, no complaints",
        "Normal shipping time, okay product", "Basic functionality, adequate",
        "It's fine, not great not bad", "Ordinary product, serves its purpose",
        "Reasonable quality for price point", "Standard item, works okay",
        "Neither impressed nor disappointed", "Average experience overall",
...
def train_intent_classifier():
    ...
    intent_data = {
        "greeting": [
            ...
            "hi there", "hello there", "hey there", "greetings", "howdy", "sup",
            "what's up", "yo", "hiya", "good day", "nice to meet you",
        ],
        "goodbye": [
            "bye", "goodbye", "see you", "see ya", "later", "take care", "farewell",
            "until next time", "catch you later", "peace out", "gotta go", "bye bye",
            "have a good day", "talk later", "signing off", "cya",
        ],
        "help": [
            "help me", "I need help", "can you help", "assist me", "support please",
            "how do I", "what should I do", "guide me", "instructions please",
            "show me how", "explain how to", "help with", "assistance needed",
            "I'm stuck", "need guidance", "how does this work",
        ],
        "fraud": [
            "check fraud", "detect fraud", "fraud analysis", "suspicious transaction",
            "fraudulent activity", "is this fraud", "fraud detection", "scam check",
            "verify transaction", "fraud alert", "suspicious payment", "fraud report",
            "unauthorized transaction", "potential fraud", "fraud risk",
        ],
        "cyber": [
            "check security", "cyber threat", "network intrusion", "malware scan",
            "security analysis", "detect attack", "cyber security", "hack attempt",
            "intrusion detection", "threat analysis", "security breach", "firewall check",
            "vulnerability scan", "ddos attack", "phishing attempt",
        ],
        "behavior": [
            "user behavior", "anomaly detection", "unusual activity", "behavior analysis",
            "pattern detection", "suspicious behavior", "insider threat", "activity monitor",
            "behavior anomaly", "user monitoring", "access patterns", "deviation detection",
        ],
        "voice": [
            "voice analysis", "analyze voice", "emotion from voice", "speech emotion",
            "voice emotion", "how do I sound", "my voice", "analyze my speech",
            "detect emotion in voice", "voice sentiment", "audio emotion",
        ],
        "status": [
            "system status", "check status", "how is the system", "health check",
            "service status", "is everything working", "system health", "uptime",
            "diagnostics", "performance check", "system info", "status report",
        ],
        "unknown": [
            "asdfgh", "random text", "qwerty", "12345", "...", "???",
            "blah blah", "test test", "xyz abc", "nonsense here",
        ],
    }
