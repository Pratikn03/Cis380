...
def _readiness_status(checks: list[GateCheck], *, blocking: bool) -> str:
    blockers = [check.name for check in checks if check.status != "ok"]
    if not blockers:
        return READY
    if blocking:
...
def main() -> int:
    ...
    core_readiness = _readiness_status(core_checks, blocking=True)
    extended_readiness = _readiness_status(extended_checks, blocking=False)

    core_payload = {
        "generated_at": _now_utc(),
        "tier": "core",
        "status": core_status,
        "readiness_status": core_readiness,
        "blocking": core_readiness == BLOCKED,
    ...
    extended_payload = {
        ...
        "readiness_status": extended_readiness,
        "blocking": extended_readiness == BLOCKED,
    ...
    print(json.dumps({"core": core_readiness, "extended": extended_readiness}, indent=2))
    return 0
