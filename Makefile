...
help:
	@echo ""
	@echo "OmniChatX / UAIS-V - Common Commands"
	@echo ""
...
streamlit:
	OMNICHATX_BACKEND=$${OMNICHATX_BACKEND:-http://localhost:8000} $(PY) -m streamlit run app/streamlit_chatbot/app.py
...
docker-build:
	docker build -f Dockerfile.production --target production -t omnichatx:prod .
