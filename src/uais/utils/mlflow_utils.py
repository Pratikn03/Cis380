...
def load_mlflow_settings(config_path: str | Path | None = None) -> Dict[str, str]:
    ...
    if not path.exists():
        return {"tracking_uri": "http://localhost:5000", "experiment_name": "Sentifargo_Experiments"}
    cfg = yaml.safe_load(path.read_text())
    ml_cfg = cfg.get("mlflow", {}) if isinstance(cfg, dict) else {}
    return {
        "tracking_uri": ml_cfg.get("tracking_uri", "http://localhost:5000"),
        "experiment_name": ml_cfg.get("experiment_name", "Sentifargo_Experiments"),
    }


def setup_mlflow(
    experiment_name: str = "Sentifargo_Experiments", tracking_uri: str | None = None
) -> None:
