...
def simple_train_val_split(
    texts: List[str], labels: List[int], test_size: float = 0.2, seed: int = 42
) -> Tuple:
...
def load_csv_text_label(
    path: str, text_col: str = "text", label_col: str = "label"
) -> Tuple[list[str], list[int]]:
    df = pd.read_csv(path)
    if text_col not in df.columns or label_col not in df.columns:
        raise KeyError(
            f"Columns {text_col} and {label_col} are required. Found: {df.columns.tolist()}"
        )
