...
class RagSettings:
    ...
    legacy_docs_dir: Path = _resolve_path(
        os.getenv("RAG_LEGACY_DOCS_DIR", "data/docs"), PROJECT_ROOT
    )
    output_dir: Path = _resolve_path(
        os.getenv("RAG_OUTPUT_DIR", "data/processed/rag"), PROJECT_ROOT
    )
    logs_path: Path = _resolve_path(
        os.getenv("RAG_LOG_PATH", "logs/rag_queries.jsonl"), PROJECT_ROOT
    )
    embed_model: str = os.getenv(
        "RAG_EMBED_MODEL", _yaml_value("embedding_model", "all-MiniLM-L6-v2")
    )
    ...
    min_avg_score: float = float(
        os.getenv("RAG_MIN_AVG_SCORE", _yaml_value("min_avg_score", "0.18"))
    )
    redact_pii: bool = _to_bool(
        os.getenv("RAG_REDACT_PII", _yaml_value("redact_pii", "true")), True
    )
