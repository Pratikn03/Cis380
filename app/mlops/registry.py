            ...
            try:
                ...
                for model_name, versions in data.items():
                    self._registry[model_name] = {
                        ver: ModelVersion.from_dict(info)
    ...
    def list_models(self) -> List[ModelInfo]:
        ...
        with self._lock:
            ...
            for name, versions in self._registry.items():
                ...
                prod_version = next(
                    (v.version for v in versions.values() if v.stage == "production"),
                    None
                )
                staging_version = next(
                    (v.version for v in versions.values() if v.stage == "staging"),
                    None
                )
