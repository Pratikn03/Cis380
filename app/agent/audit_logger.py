...
class AuditLogger:
    ...
    def __init__(
        self, log_dir: Optional[Path] = None, max_memory_entries: int = 10000, persist: bool = True
    ):
        """
    ...
    def _update_stats(
        self, route: Optional[str] = None, latency_ms: Optional[float] = None, error: bool = False
    ) -> None:
    ...
    def log_response(
        ...
        entry = AuditEntry(
            ...
            timestamp=self._now(),
            event_type="response",
            user_id=self._pending_requests.get(
                request_id,
                AuditEntry(
                    id="",
                    timestamp="",
    ...
    def get_user_history(self, user_id: str, n: int = 50) -> List[Dict]:
        ...
        with self._lock:
            user_entries = [e for e in self._entries if e.user_id == user_id][-n:]
...
__all__ = ["AuditLogger", "AuditEntry", "get_audit_logger"]
