...
class IntentScore:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "alternatives": [
                {"intent": i, "confidence": round(c, 4)} for i, c in self.alternative_intents[:3]
            ],
        }
...
class IntentConfidenceScorer:
    ...
    INTENT_PATTERNS: Dict[str, List[str]] = {
        ...
        "behavior": [
            r"user.*behavior",
            r"unusual.*activity",
            r"behavioral.*anomaly",
            r"insider.*threat",
            r"employee.*monitoring",
        ],
        "voice_emotion": [
            r"voice.*emotion",
            r"detect.*emotion.*audio",
            r"analyze.*speech",
            r"emotional.*state",
            r"tone.*analysis",
        ],
...
__all__ = ["IntentConfidenceScorer", "IntentScore", "get_confidence_scorer", "score_intent"]
