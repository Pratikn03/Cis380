...
class LatencyMonitor:
    ...
    def __init__(self, max_samples: int = 10000, window_minutes: int = 60):
        """
    ...
    def record(
        self, name: str, latency_ms: float, success: bool = True, metadata: Optional[Dict] = None
    ) -> None:
        """
    ...
    def _get_recent_samples(self, name: str, window_minutes: Optional[int] = None) -> List[float]:
        ...
        with self._lock:
            ...
            return [lat for ts, lat, _, _ in samples if ts >= cutoff_str]

    def get_stats(self, name: str, window_minutes: Optional[int] = None) -> Optional[LatencyStats]:
        """
    ...
    def get_slow_operations(
        self, threshold_ms: float = 1000, window_minutes: Optional[int] = None
    ) -> List[Dict]:
    ...
    def detect_anomalies(self, name: str, z_threshold: float = 3.0) -> List[Dict]:
        """
    ...
    def get_timeline(
        self, name: str, bucket_minutes: int = 5, window_minutes: Optional[int] = None
    ) -> List[Dict]:
        """
        ...
        with self._lock:
            samples = [
                (ts, lat)
                for ts, lat, _, _ in self._samples.get(name, [])
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_operations": [op["name"] for op in slow_ops if op["severity"] == "critical"],
