...
class FraudDataGenerator:
    ...
    def __init__(self, seed: int = 42, n_users: int = 1000, n_merchants: int = 500):
        """
    ...
    def _generate_user_profiles(self) -> Dict[str, UserProfile]:
        ...
        for i in range(self.n_users):
            ...
            # Typical behaviors
            typical_merchants = random.sample(self.MERCHANT_CATEGORIES, k=random.randint(3, 8))
    ...
    def generate_legitimate_transaction(
        self, user: UserProfile, timestamp: datetime
    ) -> Transaction:
        ...
        distance = self._calculate_distance(
            user.home_lat, user.home_lng, location_lat, location_lng
        )
    ...
    def generate_fraud_transaction(
        self, user: UserProfile, timestamp: datetime, fraud_type: FraudType
    ) -> Transaction:
        ...
        if fraud_type == FraudType.CARD_NOT_PRESENT:
            ...
            txn.device_type = random.choice(
                [d for d in self.DEVICE_TYPES if d not in user.device_ids]
            )
            ...
            txn.distance_from_home = self._calculate_distance(
                user.home_lat, user.home_lng, txn.location_lat, txn.location_lng
            )
            ...
            unusual_categories = [
                c for c in self.MERCHANT_CATEGORIES if c not in user.typical_merchants
            ]
    ...
    def save(self, transactions: List[Transaction], path: str, format: str = "csv") -> None:
        """
