...
4. **Environment variables** (minimum):
   ...
   - `AUTH_TOKEN` only if you still expose legacy token-auth routes
...
- The backend must return healthy `/health/ready` checks before you treat the deployment as live.
