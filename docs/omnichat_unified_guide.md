# Streamlit UI Guide (OmniChatX)

OmniChatX ships with a Streamlit “command center” UI that exercises most of the FastAPI endpoints from one place.
...
- `OMNICHATX_BACKEND` — FastAPI base URL (default is `http://localhost:8000`)
...
- **ChatGPT‑style chat**: chat UI variant (useful for testing frontend behaviors).
...
OmniChatX can run without any external LLM:
- If `OPENAI_API_KEY` is **unset**, `/api/chat` uses an offline response path plus local RAG.
- If `OPENAI_API_KEY` is **set**, chat routes may call OpenAI (see `agent/orchestrator.py`).

## Common Issues

### UI can’t reach backend
- Confirm `OMNICHATX_BACKEND` points to the running API.
